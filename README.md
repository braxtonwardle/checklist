...
The on-screen item labels (e.g. what "Chess study" or "Household project" is actually called in the UI) are personalized to taste and live only in the item config objects near the top of the `<script>` block — the descriptions above use the underlying, generic meaning of each item so the feature set makes sense independent of naming. Swap the `label` fields for whatever's meaningful to you; the `key` fields are the stable identifiers everything else (storage, streaks) depends on, so leave those alone.
